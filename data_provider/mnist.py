...
class InputHandle:
    def __init__(self, input_param):
        ...
        self.mini_batch_size = input_param['mini_batch_size']
    ...
    def load(self):
        ...
        # keys는 'clips', 'input_raw_data', 'output_raw_data'
        for key in dat_1.keys():
            self.data[key] = dat_1[key]

        # 데이터가 경로 2개에 걸쳐 저장될 경우 load 후 concat 방식으로 붙임
        # 지금 우리는 이 if문 안으로 들어가지 않음
        if self.num_paths == 2:
    ...
    def begin(self, do_shuffle=True):
        ...
        # 다음 데이터셋의 출력이 배치 크기보다 작을 경우엔 남은 데이터셋의 크기만 내보냄
        if self.current_position + self.mini_batch_size <= self.total():
            self.current_batch_size = self.mini_batch_size
        else:
    ...
    def input_batch(self):
        ...
        # we changed to pytorch so we don't need it
